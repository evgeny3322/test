...
export class StripeService {
  ...
  async getPublicKey(): Promise<string | null> {
    ...
    try {
      ...
      if (response.data && response.data.data && response.data.data.publicKey) {
        ...
        this.publicKey = 'pk_test_51QFZCWEFcBt0NGMG3Z8dSfot6WiiCdLvdyjtw2rMj8P6Ct6KWLJuaRcvzAywIhB3ox899bpCcOHw3t8YRjSy4wni00iPfOMnzO';
   ...
   */
  async initCardElement(elementId: string = '#payment-element'): Promise<boolean> {
    if (!this.stripe) {
  ...
  async processPayment(
    ...
    userDetails: PaymentUser,
    isAuthenticated: boolean
  ): Promise<{ success: boolean; orderId?: number; error?: string; paymentResult?: any }> {
    try {
      // 1. Create order
